...
const ENTITY_TYPE = DraftCreateRequestEntityType.WORKOUT;
...
export default function CreateWorkoutScreen() {
    ...
    useEffect(() => {
        ...
        if (drafts.length === 0) {
            setRedirected(true);
            router.replace({pathname: '/workout/draft/[draftId]', params: {draftId: 'new', type: 'quick'}});
        }
    }, [isLoading, drafts, redirected]);

    function openDraft(draft: DraftResponse) {
        router.push({pathname: '/workout/draft/[draftId]', params: {draftId: String(draft.id), type: 'quick'}});
    }

    function startNew() {
        router.push({pathname: '/workout/draft/[draftId]', params: {draftId: 'new', type: 'quick'}});
    }

    if (isLoading || (drafts.length === 0 && !redirected)) {
        return (
            <DetailLayout title="Quick Workout">
    ...
    return (
        <DetailLayout title="Quick Workout" headerRight={
            ...
            <View className="flex-1 bg-background">
                <View className="px-6 pt-6 pb-4">
                    <Typography variant="muted">
                        You have saved drafts. Continue one or start fresh.
                    </Typography>
                </View>
                ...
                <View className="px-6 pt-6">
                    <Button label="New quick workout" onPress={startNew}/>
                </View>
            </View>
        </DetailLayout>
    );
