/**
 * Generated by orval v7.13.2 🍺
 * Do not edit manually.
 * Fitness App API
 * Comprehensive API for fitness application with support for workout plans, exercises, and progress tracking. API provides OAuth2 authentication via Keycloak or JWT tokens.
 */
import type { DraftResponseEntityType } from './draftResponseEntityType';
import type { DraftResponseContent } from './draftResponseContent';

export interface DraftResponse {
  id?: number;
  profileId?: number;
  entityType?: DraftResponseEntityType;
  title?: string;
  content?: DraftResponseContent;
}
